...
//▣ 출력설명
//첫 번째 줄에 심바가 토끼를 잡아먹고 삼촌 스카에게 복수를 하러갈 수 있는 시간을 출력한 다.
...
int main() {
    ...
    while (!q.empty()) {
        ...
        for(int i=0;i<4;i++){
            int nextx = x+dx[i];
            int nexty = y+dy[i];
            if(nextx<0 || nexty<0 || nextx>=n || nexty>=n || map[nextx][nexty]>simba.size || ch[nextx][nexty]>0 ) continue;
            q.push(State(nextx,nexty,dis+1));
            ch[nextx][nexty]=1;
        }
    }
